<template>
	<view>
		...
		<view class="form re">
			...
			<view class="username">
				<input placeholder="请输入用户名" v-model="user_name" placeholder-style="color: rgba(255,255,255,0.8);"/>
			</view>
			<view class="code">
				<input placeholder="请输入电话号码" v-model="telephone" placeholder-style="color: rgba(255,255,255,0.8);"/>
			</view>
			<view class="code">
				<input placeholder="请输入账号" v-model="account" placeholder-style="color: rgba(255,255,255,0.8);"/>
			</view>
			<view class="password">
	...
	export default {
		...
		methods: {
			...
			doReg(){
				...
				setTimeout(()=>{
					uni.request({
					    url: common.websiteUrl + '/api/register', 
						method: 'POST',
						data: {
					    ...
					    success: (res) => {
							...
							if(res.data.code === 200){
								...
								uni.hideLoading()
								uni.showToast({title: '注册失败',icon:"fail"});
