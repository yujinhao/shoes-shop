<template>
	<view>
		...
		<view class="user">
			<!-- 头像 -->
			<view class="left">
				<image :src="isLogin == false ? user.head_img : url + user.head_img" @tap="toLogin"></image>
			</view>
			<!-- 昵称,个性签名 -->
			<view class="right">
				<view class="username" @tap="toLogin">{{user.user_name}}</view>
				<view class="signature" @tap="toLogin">{{user.telephone}}</view>
			</view>
			<!-- 二维码按钮 -->
		...
		<!-- 订单-余额 -->
		<view class="order" v-if="isLogin==true">
			<!-- 订单类型 -->
			<view class="list">
		...
		<!-- 工具栏 -->
		<view class="toolbar" v-if="isLogin==true">
	...
	export default {
		data() {
			return {
				...
				user:{
					user_name:'未登录',
					head_img:'/static/img/face.jpg',
					telephone:'点击跳转登录/注册页',
					integral:0,
					balance:0,
					envelope:0
				},
				...
				mytoolbarList:[
					...
					{url:'',text:'用户设置',img:'/static/img/user/security.png'},
		...
		methods: {
			...
			toPage(url){
				if(!url){
					uni.showToast({title: '未开发',icon:"none"});return;
				}
				uni.navigateTo({
					url:url
				})
...
<style lang="scss">
	...
	.order{
		...
		.list{
			display: flex;
			border-bottom: solid 1upx #5555ff;
