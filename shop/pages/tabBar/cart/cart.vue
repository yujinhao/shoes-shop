<template>
	<view>
		...
		<!-- 商品列表 -->
		<view class="goods-list" v-if="!isLogin">
			<view class="tis" >请先登录~</view>
		</view>
		<view class="goods-list" v-if="isLogin">
			<view class="tis" v-if="goodsList.length==0">购物车是空的哦~</view>
            <view class="row" v-for="(row,index) in goodsList" :key="index" >
				<!-- 删除按钮 -->
				<view class="menu" @tap.stop="deleteGoods(row.product_id)">
				...
				<view class="carrier" :class="[theIndex==index?'open':oldIndex==index?'close':'']" @touchstart="touchStart(index,$event)" @touchmove="touchMove(index,$event)" @touchend="touchEnd(index,$event)">
					...
					<view class="goods-info" @tap="toGoods(row)">
						...
						<view class="info">
							...
							<view class="spec">{{row.color}},{{row.size}}</view>
...
import common from '../../../common/common.js';
	export default {
					...
					for(let i = 0; i < ret.data.length; i++){
						uni.request({
						    ...
						    success: (res) => {
								...
								ret.data[i].product_name = res.data.data.product_name;
								this.goodsList.push(ret.data[i])
		...
		methods: {
			...
			toGoods(e){
				uni.navigateTo({
					url: '../../goods/goods?id=' + e.product_id 
				});
			},
			//跳转确认订单页面
			toConfirmation(){
				...
				for(let i=0;i<len;i++){
					if(this.goodsList[i].selected) {
						...
						tmpList1.push(this.goodsList[i]);
					}
				}
			...
			deleteGoods(id){
				let len = this.goodsList.length;
				for(let i=0;i<len;i++){
					if(id==this.goodsList[i].product_id){
						this.selectedList.splice(this.selectedList.indexOf(this.goodsList[i]), 1);
			...
			deleteList(){
				...
				{
					this.deleteGoods(this.selectedList[0].product_id);
				}
				this.selectedList = [];
			...
			selected(index){
				...
				let i = this.selectedList.indexOf(this.goodsList[index]);
				i>-1?this.selectedList.splice(i, 1):this.selectedList.push(this.goodsList[index]);
			...
			allSelect(){
				...
				for(let i=0;i<len;i++){
					...
					arr.push(this.goodsList[i].product_id);
				}
