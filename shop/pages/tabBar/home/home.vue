	...
	<view>
		...
		<view class="category-list">
			<view
				...
				:key="index"
				@tap="toCategory(index)"
			>
				<view class="img"><image :src="url + row.logo_url"></image></view>
				<view class="text">{{ row.category_name }}</view>
			</view>
		</view>
		...
		<view class="goods-list">
			...
			<view class="product-list">
				<view
					...
					@tap="toGoods(product)"
				>
					<image mode="widthFix" :src="product.bd_product_files.length == 0 ? no_data : url + product.bd_product_files[0].url"></image>
...
export default {
	data() {
		return {
			...
			// 轮播图片
			swiperList: [
				{ id: 1, src: 'url1', img: '/static/img/banner/1.jpg' },
				{ id: 2, src: 'url2', img: '/static/img/banner/2.jpg' },
				{ id: 3, src: 'url3', img: '/static/img/banner/3.jpg' }
			],
			// 分类菜单
			categoryList: [],
			//猜你喜欢列表
	...
	onReachBottom() {
		...
		uni.request({
		    ...
		    success: (res) => {
				...
				if(this.page * this.limit >= res.data.data.count){
	...
	methods: {
		...
		getCategory(){
			uni.request({
			    url: common.websiteUrl + '/api/getCategory', 
			    success: (res) => {
